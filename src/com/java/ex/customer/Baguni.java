	...
	public Baguni() {
		...
		for (int i = 0; i < BaguniList.size(); i++) {
			...
			JLabel lblBaguni = new JLabel(
					"가게명 : " + hashmap.get("businessname") + " " + "메뉴 : " + hashmap.get("menuname") + " " + "가격 : "
							+ hashmap.get("menuprice") + "원" + "  " + "수량 : " + hashmap.get("menu_count"));
			...
			btnPlus.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					int Plus = JOptionPane.showConfirmDialog(null, "수량을 늘리시겠습니까?", "수량 추가", JOptionPane.YES_NO_OPTION);
					...
					if (Plus == JOptionPane.YES_OPTION) {
						...
						lblBaguni.setText(
								"가게명 : " + hashmap.get("businessname") + " " + "메뉴 : " + hashmap.get("menuname") + " "
										+ "가격 : " + hashmap.get("menuprice") + "원" + "  " + "수량 : " + menu_count);
					}
				}
			});
			...
			btnMinus.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					int Minus = JOptionPane.showConfirmDialog(null, "수량을 내리겠습니까?", "수량 감소 ", JOptionPane.YES_NO_OPTION);
					int menu_count = (int) hashmap.get("menu_count") - 1;
					hashmap.put("menu_count", menu_count);
					if (menu_count >= 1) {
						if (Minus == JOptionPane.YES_OPTION) {
							dao.BaguniUpdate(member.getId(), menu_count, sbno);
							lblBaguni.setText("가게명 : " + hashmap.get("businessname") + " " + "메뉴 : "
									+ hashmap.get("menuname") + " " + "가격 : " + hashmap.get("menuprice") + "원" + "  "
									+ "수량 : " + menu_count);
		...
		btnBaguni.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				repaint();
				setVisible(false);
				int pay = JOptionPane.showConfirmDialog(null, "주문하시겠습니까?", "주문 확인", JOptionPane.YES_NO_OPTION);
				...
				if (pay == JOptionPane.YES_OPTION) {
					...
					for (int i = 0; i < BaguniList.size(); i++) {
						HashMap<String, Object> hashmap = (HashMap<String, Object>) BaguniList.get(i);
						JLabel lblBaguni = new JLabel("가게명 : " + hashmap.get("businessname") + " " + "메뉴 : "
								+ hashmap.get("menuname") + " " + "가격 : " + hashmap.get("menuprice") + "원" + "  "
								+ "수량 : " + hashmap.get("menu_count"));
						lblBaguni.setBounds(posX, posY + (i * 50), 800, 50);
						lblBaguni.setFont(font3);
						...
						btnPlus.addActionListener(new ActionListener() {
							@Override
							public void actionPerformed(ActionEvent e) {
								int Plus = JOptionPane.showConfirmDialog(null, "수량을 늘리시겠습니까?", "수량 추가",
										JOptionPane.YES_NO_OPTION);
						...
						btnMinus.addActionListener(new ActionListener() {
							@Override
							public void actionPerformed(ActionEvent e) {
								int Minus = JOptionPane.showConfirmDialog(null, "수량을 내리겠습니까?", "수량 감소 ",
										JOptionPane.YES_NO_OPTION);
								int menu_count = (int) hashmap.get("menu_count") - 1;
								hashmap.put("menu_count", menu_count);
								if (menu_count >= 1) {
									if (Minus == JOptionPane.YES_OPTION) {
										dao.BaguniUpdate(member.getId(), menu_count, sbno);
										lblBaguni.setText("가게명 : " + hashmap.get("businessname") + " " + "메뉴 : "
												+ hashmap.get("menuname") + " " + "가격 : " + hashmap.get("menuprice")
												+ "원" + "  " + "수량 : " + menu_count);
		...
		btnBaguniClear.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				setVisible(true);
				int clear = JOptionPane.showConfirmDialog(null, "바구니를 비우겠습니까?", "바구니 비우기", JOptionPane.YES_NO_OPTION);
				if (clear == JOptionPane.YES_OPTION) {
